...
class Pedestrian (Supervisor):
    def __init__(self):
        ...
        print('BEFORE SUPERVISOR __init__')
        try:
            Supervisor.__init__(self)
        except Exception as e:
            print(str(e))
        print('AFTER SUPERVISOR __init__')
        self.out = open('/tmp/'+self.getName()+'.wlog', 'a')
        self.out.flush()
    def print(self, x):
        self.out.write(x+'\n')
        self.out.flush()
    ...
    def run(self):
        ...
        if not options.trajectory or len(options.trajectory.split(',')) < 2:
            self.print("You should specify the trajectory using the '--trajectory' option.")
            self.print("The trajectory shoulld have at least 2 points.")
            return
        self.print(self.getName()+' 2')
        ...
        self.print(self.getName()+' got to the loop!')
...
print('pedestrian '+str(controller.getName()))
controller.print('pedestrian '+str(controller.getName()))
controller.run()
