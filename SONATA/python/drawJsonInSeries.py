...
def print_usage():
    print("Please introduce the name of the json Directory")
    print("Example: python3 ", sys.argv[0], " jsons_dir", "- count of previous data collection")


# Load json file
def load_json_frame(i,file):
    global data
    if len(data) == 0:
        ...
        # print(len(data))
        # print("All frames showed for this scene")
        data = []
        return None
    return data[i]
...
file_num = 0
for index,file in enumerate(sorted(os.listdir(sys.argv[1])[int(sys.argv[2]):])):
	file_num += 1
	frame_number = 0
	file_path = sys.argv[1]+"/"+file
	frame = load_json_frame(frame_number,file_path)
	print_scene(frame)
...
print("Number of scenes checked: "+str(file_num))